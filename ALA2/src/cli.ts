...
import { Tarea, Estado, Dificultad, SortField } from "./interfaces";

export class CLI {
  private service: Tareas;
  private userName: string = "";

  constructor(service: Tareas) {
  ...
  public run(): void {
    ...
    while (true) {
      ...
      switch (opcion) {
        case "1": this.menuVerTareas(); break;
        case "2": this.menuBuscarTarea(); break;
        case "3": this.agregarTarea(); break;
        case "4": console.log(`¡Hasta luego ${this.userName}!`); return;
  ...
  private menuVerTareas(): void {
    ...
    let filtro: Estado | 'todas' = 'todas';

    if (filtroOpcion === "2") filtro = Estado.Pendiente;
    else if (filtroOpcion === "3") filtro = Estado.EnCurso;
    else if (filtroOpcion === "4") filtro = Estado.Terminada;
    else if (filtroOpcion === "0") return;
  ...
  private menuDetallesTarea(tarea: Tarea): void {
    while (true) {
      ...
      if (opcion === "0") return;
      if (opcion === "e") this.menuEdicion(tarea);
    }
  }

  private menuEdicion(tarea: Tarea): void {
  ...
  private menuOrdenacion(): SortField {
  ...
  private listarTareas(filtro: Estado | 'todas'): void {
  ...
  private agregarTarea(): void {
    ...
    let vencimiento: string | null = readline.question("5. Fecha de vencimiento (YYYY-MM-DD) o vacío: ");
